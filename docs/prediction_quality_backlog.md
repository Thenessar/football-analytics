...
| --- | --- | --- | --- | --- |
| FA-101 | P1 — Current player-Elo state for future fixtures | Ledger | — | ✅ DONE (2026-07-10; live spread recorded 2026-07-12) |
...
| FA-107 | P5 — Follow-up reruns, retrain + re-register | Registrar | FA-101, FA-102, FA-104 | ✅ DONE (2026-07-12) — live fixture verified, Epic P complete |
...
| FA-109 | P7 — Per-position-group α_player (closes the open passes PIT trigger) | Croupier | FA-108 (adopted layer) | ✅ CLOSED — NOT ADOPTED (2026-07-12): hypothesis falsified, fitted group alphas near-identical; flag stays off, ADR trigger stays open |
...
- **Progress (2026-07-12) — tasks 1–3 done, task 4 awaits the next live fixture:**
  ...
  4. **Live-fixture verification (2026-07-12, France vs Spain) — DONE, Epic P complete.** Simulated game view from the new versions' active prediction set:
     - **Player-differentiated spread restored** (the FA-101/E-2 acceptance number): Mbappé **2.06** sim-mean shots vs France starter median ≈ 0.55 — **3.8× spread** (July 9 baseline: flat 1.086, top ≈ median). Spain: Oyarzabal 1.58 vs starter median ≈ 0.53 (~3.0×). Within-team ordering is football-sensible (finishers lead shots; Rodri/Laporte/Cubarsí lead passes at ~64 vs Mbappé's 25; GKs get saves only; bench means participation-weighted).
     - **Structural identities exact in the output**: fouls mirror (10.971 committed = 10.971 drawn), saves identity (4.78 − 1.59 = 3.19 = GK saves, both sides), assists/goals = 0.689 ≈ the calibrated 0.688.
     - **Fixture coherence**: Spain slight favorite (1.59 vs 1.40 goals), top scoreline 1-1 at 10.4%; near-equal totals are correct for an even elite fixture — no compression artifact.
     - All four FA-107 tasks complete → **Epic P closed**. Remaining open threads live outside this epic: the ADR 0005 passes-PIT trigger (role-heterogeneity falsified in FA-109; next hypothesis needs PIT-bin-shape evidence) and the FA-101-notes lineup-strength follow-up.
...
- **Revalidation result (2026-07-12): NOT ADOPTED — the role-heterogeneity hypothesis is falsified.** Retrain minted per-group-tagged versions (boosters bit-identical to the FA-107 versions — same data/params/seed — e.g. passes_total v8); the N6 arm with `--player-dispersion-by-position` measured passes coverage **0.708** / PIT **0.141** vs the pooled arm's 0.710 / 0.139 — pure noise (the flag was demonstrably active: downstream draw streams shifted). Root cause is visible in the fit itself: the per-group passes alphas are near-identical (pooled 0.094; D 0.096, M 0.096, F 0.082, G 0.053) — conditional on the model's mean, passes dispersion is role-homogeneous, so no per-group mix can move the PIT. **Decision per the pre-committed rule: flag stays off everywhere (defaults already False), the machinery stays in the engine (harmless, digest-gated), and the ADR 0005 passes trigger REMAINS OPEN** with a falsified-hypothesis record. The residual ~0.14 PIT deviation is structure the NB-mixture family cannot express at any alpha mix — the leading remaining hypothesis is within-match tempo/game-state covariance (ADR 0005 v1 simplifications 1/3, which already carry their own revisit triggers); pursue only with dedicated evidence (e.g. PIT-bin shape analysis from the N6 artifacts) rather than another dispersion knob.
